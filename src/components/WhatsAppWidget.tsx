...
const OPTIONS: WaOption[] = [
  { label: "Demander un devis", message: "Bonjour, je souhaite demander un devis.", goto: "/devis" },
  {
    label: "Question sur une intervention",
    message: "Bonjour, j'ai une question sur une intervention.",
  },
  { label: "Être rappelé", message: "Bonjour, pouvez-vous me rappeler ?" },
  {
    label: "Localisation / zone d'intervention",
    message: "Bonjour, intervenez-vous dans ma zone ?",
    goto: "/zones",
  },
];
...
export function WhatsAppWidget() {
  ...
  return (
    <div className="fixed right-4 bottom-24 z-50 flex flex-col items-end gap-3 lg:bottom-6">
      {open && (
        <div className="w-[19rem] overflow-hidden rounded-2xl border border-border bg-popover shadow-lift">
          ...
          <div className="flex flex-col gap-1.5 p-3">
            {OPTIONS.map((o) => (
              <button
                ...
                type="button"
                onClick={() => handle(o)}
                className="rounded-lg border border-border px-4 py-3 text-left text-sm font-medium transition-colors hover:border-accent hover:bg-secondary"
              >
                {o.label}
              </button>
            ))}
      ...
      <button
        ...
        className="flex h-13 w-13 items-center justify-center rounded-full bg-[oklch(0.68_0.15_150)] text-white shadow-lift transition-transform hover:-translate-y-0.5 active:scale-95"
      >
